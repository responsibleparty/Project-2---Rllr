...
const buildMyCard = (poke, index) => {
    ...
    <img class='animate__animated animate__infinite animate__slower animate__pulse'
        src="/images/pokemons/${poke.api_id}.svg">
    <h2 class="poke-name">${capitalize(poke.name)}</h2>
    <div class="types">
       
        ${poke.types.map(type=>`<span style="background-color:${one_theme(type)};">${capitalize(type)}</span>`)}
    </div>
    <div class="stats">
        <div>
        ...
        <div>
            <h3>${poke.stat_att} <i class="fa-solid fa-location-crosshairs"></i></h3>
            <p>ATT</p>
        </div>
        <div>
    ...
    <div class="stats">
        ...
        <div>
            <h3>${poke.stat_special_att} <i class="fa-solid fa-skull-crossbones"></i></h3>
            <p>Sp. ATT</p>
        </div>
        <div>
            <h3>${poke.stat_special_def} <i class="fa-solid fa-shield-virus"></i></h3>
            <p>Sp. DEF</p>
        </div>
    </div>
    <div class="abilities">
